...
const LeaseWizard = ({
  ...
  const { data: propertiesData, loading: propertiesLoading } = useQuery(GET_ALL_PROPERTIES, {
  ...
  const { data: tenantsData, loading: tenantsLoading } = useQuery(GET_ALL_TENANTS, {
    variables: { page: { limit: 100, offset: 0 } },
  ...
  return (
    <>
      ...
      <form onSubmit={(e) => {
        e.preventDefault();
        // Pass all wizard data to parent
        const wizardData = {
          ...formData,
          selectedTenantIds: selectedTenants.map(t => t.tenant_id),
          primaryTenantId: primaryTenantId
        };
        handleSubmit(e, wizardData);
      }} className="space-y-6">
            ...
            <div>
              ...
              <div className="mb-4">
                ...
                <div className="relative">
                  ...
                  {/* Property Dropdown - Show loading state always when loading, otherwise show on focus */}
                  {(propertiesLoading || (showPropertyDropdown && !selectedProperty)) && (
                    <div className="absolute z-10 w-full mt-1 bg-white border border-neutral-300 rounded-lg shadow-lg max-h-60 overflow-y-auto">
                      {propertiesLoading ? (
                        <div className="px-4 py-6 text-center">
                          <svg className="w-8 h-8 mx-auto mb-2 text-primary-600 animate-spin" fill="none" viewBox="0 0 24 24">
                            <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"></circle>
                            <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
                          </svg>
                          <p className="text-sm text-neutral-500">Loading properties...</p>
                        </div>
                      ) : filteredPropertiesComputed.length > 0 ? (
